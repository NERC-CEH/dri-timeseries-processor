...
from new_processor.api_models.operations.flags import CoreFlagItem, CoreFlagResponse
...
class TestCoreFlagItem:
    ...
    def test_valid_core_flag(self, name: str, description: str, symbol: str, flag_id: int) -> None:
        ...
        """
        core_flag = CoreFlagItem(name=name, description=description, symbol=symbol, id=flag_id)
    ...
    def test_bad_id(self, bad_id: int) -> None:
        with pytest.raises(ValueError):
            CoreFlagItem(name="Test flag", description="Tests the tests", symbol="T", id=bad_id)
