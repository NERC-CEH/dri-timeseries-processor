...
from new_processor.cli.selection import CrossProductSelectionSpec, ExplicitSelectionSpec, SelectionOption
...
class TestExplicitSelectionSpec:
    def test_explicit_queries(self) -> None:
        ...
        queries = [
            SelectionOption(sites=["A"], variables=["TA"], periodicities=["P1D"]),
            SelectionOption(sites=["B"], variables=["RH"], periodicities=["PT30M"]),
        ]
    ...
    def test_deduplicates_identical_queries(self) -> None:
        ...
        query = SelectionOption(sites=["A"], variables=["TA"], periodicities=["P1D"])
...
class TestCrossProductSelectionSpec:
    def test_single_root_query(self) -> None:
        ...
        expected = [SelectionOption(sites, variables, periodicities)]
