...
from dritimeseriesprocessor.models.api_models.flags import CoreFlagResponse
from dritimeseriesprocessor.routers.metadata.local_loader import fetch_core_flags, load_metadata_json


def monkeypatch_package_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("dritimeseriesprocessor.routers.metadata.local_loader.PACKAGE_ROOT", tmp_path)


def monkeypatch_load_metadata_json(monkeypatch: pytest.MonkeyPatch, test_data: dict) -> MagicMock:
    mock_load = MagicMock()
    mock_load.return_value = test_data
    monkeypatch.setattr("dritimeseriesprocessor.routers.metadata.local_loader.load_metadata_json", mock_load)
    return mock_load
