...
@pytest.fixture
def writer(s3_test_helper: S3TestHelper) -> S3Writer:
...
class TestS3Writer:
    ...
    def test_s3_client_type(self, s3_test_helper: S3TestHelper) -> None:
        """Returns an object if s3_client is of type `boto3.client.s3`, otherwise
        raises an error"""
        s3_test_helper._create_hourly_test_data(datetime(2024, 1, 1), datetime(2024, 1, 10))

        # Happy path
        try:
            S3Writer(s3_test_helper.s3_client)
        except Exception as err:
            pytest.fail(f"An 'error should not have been raised when creating the S3Writer. Error: {err}")
    ...
    @pytest.mark.parametrize(
        "network,site_id,resolution,date", [("network", "BUNNY", "PT1M", datetime(2024, 2, 22, 1, 32, 14))]
    )
    def test_s3_key_builder(self, network: str, site_id: str, resolution: str, date: datetime) -> None:
    ...
    def test_group_data_by_resolution_and_site(
        self, s3_test_helper: S3TestHelper, ts_test_helper: TimeSeriesTestHelper
    ) -> None:
        """Test data correctly grouped by site and resolution."""
        input_filepath = s3_test_helper.input_dir.joinpath("write")
        output_filepath = s3_test_helper.output_dir.joinpath("write", "group_res_site")
        ...
        test_ts_ids = ts_test_helper.load_ts_ids_from_json_file(input_filepath.joinpath("processed_ts_ids.json"))
    ...
    def test_split_by_day(self, s3_test_helper: S3TestHelper) -> None:
        """Test that data is split correctly."""

        # Use an output from the group_data_by_resolution_and_site test as inputs
        input_filepath = s3_test_helper.output_dir.joinpath("write", "group_res_site")
...
class TestMergeDataframes:
    ...
    def test_two_matching_dataframes(self, writer: S3Writer) -> None:
        ...
        result = writer._merge_dataframes(existing_df, new_df)

        assert_frame_equal(result, expected_df)

    def test_additional_column(self, writer: S3Writer) -> None:
    ...
    def test_update_to_existing_column(self, writer: S3Writer) -> None:
    ...
    def test_update_to_existing_columns(self, writer: S3Writer) -> None:
    ...
    def test_different_timestamps_no_overlap(self, writer: S3Writer) -> None:
    ...
    def test_different_timestamps_overlap(self, writer: S3Writer) -> None:
    ...
    def test_different_timestamps_and_columns(self, writer: S3Writer) -> None:
